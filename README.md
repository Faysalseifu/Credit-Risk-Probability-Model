# Ìø¶ Credit Risk Probability Model (Alternative Data)
...
- **Estimated business impact**: Rejecting top ~20% proxy high-risk could reduce potential defaults by 30‚Äì45% (based on proxy correlation).

---

## Ì≥∏ Model Explainability (SHAP)
...
## Ìª°Ô∏è Business Understanding & Compliance
...
```text
‚îú‚îÄ‚îÄ artifacts/               # Model binaries, SHAP plots, and screenshots
‚îú‚îÄ‚îÄ data/                    # Raw and processed datasets (git-ignored)
‚îú‚îÄ‚îÄ notebooks/               # EDA and Model prototyping
‚îú‚îÄ‚îÄ src/
‚îÇ   ‚îú‚îÄ‚îÄ api/                 # FastAPI prediction service
‚îÇ   ‚îú‚îÄ‚îÄ data_processing.py   # Feature engineering pipeline
‚îÇ   ‚îú‚îÄ‚îÄ shap_analysis.py     # SHAP generation scripts
‚îÇ   ‚îî‚îÄ‚îÄ train.py             # Model training CLI
‚îú‚îÄ‚îÄ tests/                   # Unit & Integration tests
‚îî‚îÄ‚îÄ dashboard.py             # Streamlit browser interface
```
